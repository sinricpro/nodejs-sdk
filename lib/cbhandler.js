...
const handlePayload = (payload, signature, callbacks) => verifySignature(payload, signature).then((action) => {
  ...
  if (action === 'setPowerState') {
    ...
    return AdjustBands(payload, callbacks).then((value) => jsonResponse(payload, value)).catch((err) => {
      console.log(err.message);
    });
  } else if (action === 'resetBands') {
    return BandReset(payload, callbacks).then((value) => jsonResponse(payload, value)).catch((err) => {
      console.log(err.message);
    });
  } else if (action === 'setMode') {
    return Mode(payload, callbacks).then((value) => jsonResponse(payload, value)).catch((err) => {
      console.log(err.message);
    });
